...
pub async fn run_sandbox(
    ...
    // Shared UID policy registry — written by the mediator's fork_with_policy
    // syscall, read by the proxy's fast-path to enforce per-workflow HTTP
    // allowlists without running the full OPA identity pipeline. Unused when
    // not in proxy mode.
    let uid_registry = mediator::registry::UidPolicyRegistry::new();

    let (_proxy, denial_rx, bypass_denial_tx, proxy_addr) = if matches!(
        policy.network.mode,
        NetworkMode::Proxy
    ) {
        ...
        (
            ...
            None,
            None,
            SocketAddr::from(([127, 0, 0, 1], 3128)),
        )
    };
    ...
    // proxy share the UidPolicyRegistry so forked workflows' network
    // policies are immediately visible to the proxy fast path.
    ...
    let mediator_root_token = {
        ...
        // JSON config file. The config file is useful when env vars can't
        // be injected into the sandbox pod spec.
        ...
        match mediator::init::bootstrap_embedded(&config, uid_registry.clone()).await {
            Ok(result) => {
                ...
                // World-accessible socket so sandboxed UIDs can connect.
                #[cfg(target_os = "linux")]
                {
                ...
                // Persist the root token so the mediator-tools plugin
                // inside the OpenClaw gateway can read it (the gateway
                // may not inherit MEDIATOR_TOKEN from PID 1's env).
    ...
    // Inject mediator root token into the entrypoint's env so the process
    // can authenticate with the embedded mediator daemon via the UDS.
