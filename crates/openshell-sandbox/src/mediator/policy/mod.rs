...
pub struct MediationPolicy {
    ...
    /// Deprecated: IPC syscalls (`ipc_send`/`ipc_connect`) were dropped from
    /// the simplified mediator. Field retained so on-disk policies from the
    /// previous schema continue to deserialize; always ignored at runtime.
    #[serde(default)]
