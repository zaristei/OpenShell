...
use crate::mediator::policy::inherit;
...
pub async fn handle_fork_with_policy(
    ...
    {
        let policies_guard = policies.read().await;
        let child_policy = policies_guard.get(&workflow.policy_name);
        let parent_policy = if params.inherit {
            caller_token
                .inherited_from
                .as_deref()
                .and_then(|_| Some(caller_policy))
        } else {
            None
        };
        if let Some(cp) = child_policy {
            let resolved = inherit::resolve(cp, parent_policy);
            let net_policy = convert::to_workflow_net_policy(&resolved, &params.workflow_id);
